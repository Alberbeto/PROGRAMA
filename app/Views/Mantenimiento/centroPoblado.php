...
<div class="card mt-4">
    <div class="card-header">
        <li class="breadcrumb-item active"><i class="fa fa-users"></i> CENTRO POBLADO</li>
    </div>
...
<!-- Modal -->
<form action="<?php echo base_url('centro') ?>" method="post" id="insertar">
    <div class="modal fade" id="formModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">NUEVO C.P.</h1>
                ...
                <div class="modal-body">
                    <div class="row">
                        <input type="hidden" class="form-control"  id="idsigma" name="idsigma" required>
                            ...
                            <div class="col-sm-6">
                                ...
                                <select class="form-select" aria-label="Default select example" id="ctipcen" name="ctipcen" required>
                                <?php if(!empty($tipos)){ ?> 
                            ...
                            <div class="col-sm-6">
                                ...
                                <input type="text" class="form-control" id="cidzona" name="cidzona" required>
                            </div>
                            <div class="col-sm-6">
                                <label for="sector" class="form-label">SECTOR</label>
                                <select class="form-select" aria-label="Default select example" id="nsector" name="nsector" required>
                                <?php if(!empty($sectores)){ ?> 
                                    <?php foreach($sectores as $sector){ ?>
                                        <option value="<?php echo $sector["cid_sector"];?>"><?php echo $sector["vdescrip"];?></option>
                                    <?php }?> 
                                    <?php }else{?>
                                            <option value="">No hay sectores disponibles</option>
                                        <?php } ?>
                    ...
                    <div class="col-12 checkbox-container" style="width: 100%;">
                        ...
                        <div class="col-12 mt-4">
                            <input type="hidden" class="form-control"  id="cdistri" name="cdistri" required>
...
<script>
    ...
    function abrirmodal(json){
        ...
        if(json!=null){
            ...
            $('#ctipcen').val(json.ctipcen);
            $('#nsector').val(json.nsector);
  ...
  function listar(tabla){
        ...
        success:function(response){
            ...
            data.forEach(function(item){
                ...
                fila.append('<td>' + item.zona + '</td>');
                fila.append('<td>' + item.sector + '</td>');
                fila.append('<td>' + item.nombre_completo + '</td>');
                fila.append('<td>' + item.desde + '</td>');
                fila.append('<td>' + item.hasta + '</td>');
                fila.append('<td>' + item.estado + '</td>'); 

                // Añadir el botón de editar
                fila.append('<td><button type="button" class="btn btn-primary btn-sm btn-editar" onclick="editar(\'' + item.id + '\')"><i class="fas fa-pen"></i></button></td>');

                // Aplicar la clase 'fila-roja' si 'nestado' es '0'
                if (item.estado == '0') {
